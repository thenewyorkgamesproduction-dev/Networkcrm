...
export default function Home(){
 ...
 const visible=useMemo(()=>{const q=query.trim().toLowerCase(),terms=q.split(/\s+/).filter(Boolean);let rows=(tab==='search'&&results.length?results:people).slice();if((tab==='search'||tab==='people')&&terms.length)rows=rows.filter(x=>terms.every(term=>searchable(x).includes(term)));if(sort==='affinity')rows.sort((a,b)=>affinity(b)-affinity(a));else if(sort==='name')rows.sort((a,b)=>String(a.person.name||'').localeCompare(String(b.person.name||'')));else if(tab==='search'&&terms.length)rows.sort((a,b)=>localScore(b,terms)-localScore(a,terms));else rows.sort((a,b)=>affinity(b)-affinity(a)||b.memories_count-a.memories_count);return rows},[people,results,query,sort,tab]);
